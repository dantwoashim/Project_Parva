# Files Changed

This report groups the roadmap changes by workstream.

## Code

- `backend/app/future_bs/astronomy_confidence.py`
- `backend/app/future_bs/ephemeris/__init__.py`
- `backend/app/future_bs/ephemeris/jpl_spice_adapter.py`
- `scripts/ephemeris/verify_kernel_hashes.py`
- `scripts/release/check_repo_hygiene.py`

## Tests

- `packages/parva-agent-tools/tests/conftest.py`
- `packages/parva-agent-tools/tests/test_safety.py`
- `packages/parva-mcp-server/tests/conftest.py`
- `packages/parva-mcp-server/tests/test_manifest.py`

## Docs

- `docs/QUICKSTART.md`
- `docs/SDK_STRATEGY.md`
- `docs/API_VERSIONING_AND_DEPRECATION.md`
- `docs/ERROR_MODEL.md`
- `docs/DEVELOPER_EXPERIENCE.md`
- `docs/ROUTE_PROFILES.md`
- `docs/README.md`
- `docs/CURRENT_FEATURES.md`
- `packages/parva-python/README.md`
- `packages/parva-js/README.md`

## Reports

- `reports/phase_07_future_bs_governance/module_classification.md`
- `reports/phase_09_frontend_sdk_dx/adoption_hardening_report.md`
- `reports/red_check_closure/README.md`
- `reports/next_roadmap_execution/*`
- `reports/external_reviewer_packet/*`

## Benchmark

- `public-benchmark/README.md`
- `public-benchmark/benchmark.json`
- `public-benchmark/runners/run_against_parva.py`
- `public-benchmark/runners/run_against_static_baseline.py`
- `docs/reports/AGENTS_SHOULD_NOT_GUESS_NEPALI_TIME.md`

## JPL And Ephemeris

- `docs/JPL_DE440_INTEGRATION.md`
- `docs/EPHEMERIS_KERNEL_POLICY.md`
- `config/ephemeris-kernels.yaml`
- `reports/ephemeris_accuracy/README.md`
- `reports/ephemeris_accuracy/solar_ingress_jpl_vs_swiss.md`
- `reports/ephemeris_accuracy/solar_ingress_jpl_vs_swiss.json`
- `reports/ephemeris_accuracy/boundary_risk_summary.md`

## External Rules

- `config/external-temporal-rules.yaml`
- `docs/EXTERNAL_TEMPORAL_RULES.md`
- `docs/INSTITUTION_RULE_PROFILES.md`
- `rules/institution_profiles/README.md`
- `rules/festival_authority/README.md`
- `rules/holiday_authority/README.md`
- `reports/external_rules_audit.md`

## Agent Tools

- `packages/parva-agent-tools/pyproject.toml`
- `packages/parva-agent-tools/parva_tools/__init__.py`
- `packages/parva-agent-tools/parva_tools/schemas.py`
- `packages/parva-agent-tools/parva_tools/safety.py`
- `packages/parva-agent-tools/parva_tools/langchain.py`
- `packages/parva-agent-tools/parva_tools/llamaindex.py`
- `packages/parva-agent-tools/README.md`

## MCP

- `packages/parva-mcp-server/pyproject.toml`
- `packages/parva-mcp-server/src/parva_mcp_server/__init__.py`
- `packages/parva-mcp-server/src/parva_mcp_server/manifest.py`
- `packages/parva-mcp-server/src/parva_mcp_server/server.py`
- `packages/parva-mcp-server/README.md`
- `docs/MCP_INTEGRATION.md`
- `docs/MCP_SECURITY_POLICY.md`

## Government Packet

- `docs/government/NATIONAL_DIGITAL_PANCHANGA_PROPOSAL.md`
- `docs/government/ONE_PAGE_GOVERNMENT_BRIEF.md`
- `docs/government/TECHNICAL_APPENDIX.md`
- `docs/government/AUTHORITY_BOUNDARY.md`
- `docs/government/PILOT_DELIVERABLES.md`
- `docs/government/MACHINE_READABLE_RELEASE_SAMPLE.md`
- `docs/government/CONFORMANCE_FOR_VENDORS.md`
- `samples/digital-panchanga-release/2083-bs/*`

## Vendor And Commercial Packet

- `docs/VENDOR_DATE_RISK_AUDIT.md`
- `docs/TEMPORAL_SBOM_SERVICE.md`
- `docs/ENTERPRISE_DEPLOYMENT.md`
- `docs/COMMERCIAL_LICENSE.md`
- `docs/PRICING_STRATEGY.md`
- `docs/CONFORMANCE_PROGRAM.md`
- `samples/vendor-date-risk-audit/input_sample.csv`
- `samples/vendor-date-risk-audit/output_report_sample.md`
- `samples/vendor-date-risk-audit/output_report_sample.json`
