...
def days_in_bs_month(year: int, month: int) -> int:
    ...
    if data is None:
        raise ValueError(f"BS year {year} is not in supported range ({BS_MIN_YEAR}-{BS_MAX_YEAR})")
...
def _bs_to_gregorian_estimated(year: int, month: int, day: int) -> date:
    ...
    if year < min_year or year > max_year:
        raise ValueError(f"BS year {year} is outside estimated range ({min_year}-{max_year})")
    if month < 1 or month > 12:
        raise ValueError(f"Invalid month: {month}. Must be 1-12.")
    if day < 1 or day > 32:
    ...
    if day > month_length:
        raise ValueError(
            f"Invalid day {day} for estimated BS month {year}-{month:02d} " f"(max {month_length})"
        )
...
def _get_bs_month_starts_estimated(bs_year: int) -> list[tuple[int, date]]:
    ...
    if bs_year < min_year or bs_year > max_year:
        raise ValueError(f"BS year {bs_year} is outside estimated range ({min_year}-{max_year})")
...
def _gregorian_to_bs_estimated(gregorian_date: date) -> tuple[int, int, int]:
    ...
    sankrantis = get_sankrantis_in_year(start_mesh_year) + get_sankrantis_in_year(
        start_mesh_year + 1
    )
    month_starts = []
    for s in sankrantis:
