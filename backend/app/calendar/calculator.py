...
def calculate_festival_date(
    festival_id: str, gregorian_year: int, use_overrides: bool = True
) -> DateRange:
    """
...
def _calculate_solar_festival(
    festival_id: str, gregorian_year: int, rule: CalendarRule
) -> DateRange:
...
def _calculate_lunar_festival(
    ...
    if rule.tithi is None or rule.paksha is None or rule.bs_month is None:
        raise FestivalCalculationError(
            f"Lunar festival {festival_id} missing tithi, paksha, or month"
        )
    ...
    festival_start_dt = find_next_tithi(
        rule.tithi, rule.paksha, after=search_start_dt, within_days=45
    )
...
def get_upcoming_festivals(
    from_date: date, days: int = 30, festival_ids: Optional[List[str]] = None
