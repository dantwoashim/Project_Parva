---
status: research
tier: 3
lane: research
last_verified: 2026-08-05
owner: research-team
---

# Future BS Public Claims Policy

Public Future-BS language must describe research posture, not official calendar
authority.

## Allowed Public Claims

- Parva exposes public capability, methodology, and curated single-year Future-BS forecasts.
- Parva labels unpublished Future-BS outputs as computed predictions, not
  official publication.
- Parva separates official, printed, publisher, software-table, third-party,
  needs-review, and experimental evidence.
- Parva reports risk posture, source policy, warnings, and claim boundaries.
- Parva reports `72/72` only as chronological rolling validation over the official `2078-2083 BS` development window, with past-only training for each target year.
- Parva can support private research review when explicitly deployed under a
  private route profile.

## Disallowed Public Claims

- Parva guarantees future BS dates.
- Parva publishes the official future BS calendar.
- Parva replaces Panchanga Nirnayak Samiti or any legal authority.
- Parva has solved future calendar publication.
- A GREEN label means official certainty.
- Weak third-party, app, or shadow rows prove official accuracy.
- A broad all-reference stress test proves official-grade accuracy.

## Required Label

Every unpublished Future-BS prediction, risk review, or model-risk payload must
preserve:

```text
computed_prediction_not_official
```

Public copy may mention accuracy only with source policy, evidence window, corpus
size, coverage, wrong-GREEN count, false-confidence rate, and the limits of the
claim.

The public forecast must also carry `review_required=true` and
`authoritative_publication_overrides=true`.

Heuristic estimates must be named as `heuristic_accuracy_estimate` or
`heuristic_not_empirically_calibrated`. Normalized model support must not be described as calibrated probability or measured future accuracy.
