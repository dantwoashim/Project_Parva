...
| --- | --- | --- |
| Solar-ingress civil decision model | Primary computational tower | Metadata only |
| Past-only statistical pattern stack | Secondary bounded signal | Metadata only |
...
| Risk threshold classifier | GREEN/YELLOW/RED posture | Public vocabulary only |
...
- Public routes must not expose exact future vectors or model-run internals.
