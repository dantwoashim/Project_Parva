#!/usr/bin/env python3
"""Generate the public beta evaluator dossier with reproducible evidence artifacts."""
...
REPORTS_RELEASE_JSON = REPORTS_RELEASE_DIR / "public_beta_dossier.json"
REPORTS_RELEASE_MD = REPORTS_RELEASE_DIR / "public_beta_release_dossier.md"
...
def _validate_quality_gates(payload: dict) -> None:
    ...
    if missing:
        raise ArtifactError(f"Public beta dossier has null quality-gate fields: {', '.join(missing)}")
...
def _build_payload() -> dict:
    ...
    payload = {
        ...
        "quality_gates": {
            ...
            "authority_dashboard": {
                "conformance_pass_rate": authority.get("pipeline_health", {}).get(
                    "conformance_pass_rate"
                ),
                "catalog_total": authority.get("pipeline_health", {}).get("rule_catalog_total"),
                "catalog_coverage_pct": authority.get("pipeline_health", {}).get(
        ...
        "artifacts": {
            "conformance_report": _rel(PROJECT_ROOT / "reports" / "conformance_report.json"),
            "authority_dashboard": _rel(PROJECT_ROOT / "reports" / "authority_dashboard.json"),
            "accuracy_report": _rel(
                PROJECT_ROOT / "reports" / "accuracy" / "annual_accuracy_2082.json"
            ),
            "coverage_catalog": _rel(
                PROJECT_ROOT / "data" / "festivals" / "festival_rules_v4.json"
            ),
        },
    }
        ...
        json.dumps(
            {
                ...
                .get("computed", {})
                .get("count"),
                "sota_badge_allowed": payload.get("plugin_quality", {})
                .get("global", {})
                .get("sota_badge_allowed"),
            },
            indent=2,
        )
    )
