#!/usr/bin/env python3
"""Run launch-critical golden browser journeys against a built frontend served by FastAPI."""
...
def _wait_for_http(url: str, process: subprocess.Popen[str] | None, timeout_seconds: int) -> None:
    ...
    while time.time() < deadline:
        if process and process.poll() is not None:
            raise RuntimeError("Backend exited before golden journeys started.")
        try:
...
def _run_playwright_golden_journeys(base_url: str, report_path: Path) -> int:
    ...
    env = os.environ.copy()
    env["PARVA_GOLDEN_BASE_URL"] = base_url
    env["PARVA_GOLDEN_REPORT_PATH"] = str(report_path)
    command = [node_path, "scripts/golden_journeys.mjs"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run golden browser journeys against the Parva app.")
    ...
    if args.base_url:
        ...
        result = _run_playwright_golden_journeys(base_url, report_path)
        _write_report(
            report_path,
            ...
            error=None if result == 0 else "Golden journeys command returned a non-zero exit code.",
        )
        return result
