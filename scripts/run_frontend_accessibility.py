#!/usr/bin/env python3
"""Run live frontend accessibility walkthroughs against the built app."""
...
DEFAULT_REPORT_PATH = PROJECT_ROOT / "reports" / "release" / "frontend_accessibility.json"
...
def _wait_for_http(url: str, process: subprocess.Popen[str] | None, timeout_seconds: int) -> None:
    ...
    while time.time() < deadline:
        if process and process.poll() is not None:
            raise RuntimeError("Backend exited before accessibility walkthrough started.")
        try:
...
def _ensure_playwright_browser() -> int:
    ...
    if not executable_path or not Path(executable_path).exists():
        install = subprocess.run([str(playwright_cli), "install", "chromium"], cwd=FRONTEND_ROOT, check=False)
        return install.returncode

    return 0


def _run_accessibility(base_url: str, report_path: Path) -> int:
    ...
    node_path = shutil.which("node")
    env = os.environ.copy()
    env["PARVA_A11Y_BASE_URL"] = base_url
    env["PARVA_A11Y_REPORT_PATH"] = str(report_path)
    completed = subprocess.run([node_path, "scripts/accessibility_walkthrough.mjs"], cwd=FRONTEND_ROOT, env=env, check=False)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run frontend accessibility walkthroughs against Parva.")
    parser.add_argument("--base-url", help="Use an already running app instead of starting a local server.")
    parser.add_argument("--timeout-seconds", type=int, default=30)
    parser.add_argument(
        "--report-path",
        default=str(DEFAULT_REPORT_PATH),
        help="Optional JSON report path for accessibility status and artifacts.",
    )
    ...
    if args.base_url:
        ...
        result = _run_accessibility(base_url, report_path)
        _write_report(
            report_path,
            ...
            error=None if result == 0 else "Accessibility walkthrough returned a non-zero exit code.",
        )
        return result
